...
where $Y^s_i$ denotes the observed count
for a species for MBBS Orange County route
$i = 1, \dots, 11$  and $X_i = 0, \dots, t$
is the time (in years) since the start of the MBBS.
...
Let $\mu^s(\beta) = log(E[Y_i|X_i = x]) = \beta_0 + \beta_1 x$. 
Under this model, $\theta^s(x) = \beta_1$. 
We estimate $\beta$ using the generalized estimating equation:
